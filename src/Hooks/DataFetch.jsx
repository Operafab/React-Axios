...
const DataFetch = () => {
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      {loading && <p className="text-lg font-semibold text-center text-lime-700">Loading...</p>}
      {error && <p className="text-red-500 text-center">{error}</p>}

      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
        {data.map((user) => (
          <div key={user} className="bg-slate-400 shadow-md rounded-lg p-6 hover:shadow-black cursor-pointer">
            <img
